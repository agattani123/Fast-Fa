...
The server-side components of this project handle the core functionality of generating personalized scholarship opportunities based on user input. The main server file, `server.js`, sets up an Express.js server and defines the necessary routes and logic for processing user applications and fetching relevant scholarship information from the OpenAI API.

The server utilizes the OpenAI GPT-4 language model to generate tailored scholarship recommendations based on the user's provided financial information. The generated output is then rendered as an HTML page with a list of scholarship opportunities, including details such as the scholarship name, why the user is a good fit, the prize amount, the deadline, and a link to apply.
...
Sources: [server.js:1-10](), [server.js:48-50]()

### OpenAI API Integration

The server integrates with the OpenAI API to generate scholarship recommendations using the GPT-4 language model. The `fetchFromOpenAI` function is responsible for making the API request:
...
The `generateText` function is responsible for constructing the prompt and sending it to the OpenAI API for text generation:
...
Sources: [server.js:12-24](), [server.js:26-37]()

## Application Submission and Processing

The server defines a route `/submit-application` that handles the submission of user applications:
...
The route extracts the `firstName`, `lastName`, and `financial_info` from the request body. The `financial_info` is used as the prompt for the `generateText` function, which generates a list of scholarship opportunities tailored to the user's financial information.

The generated output is then modified to replace the "APPLY" text with an HTML button element, and the entire HTML page is rendered and sent as the response.

Sources: [server.js:39-116]()

## Data Flow and Sequence Diagram

The following sequence diagram illustrates the data flow and interactions between the server components and the OpenAI API:
...
sequenceDiagram
    ...
    Server->>Server: Extract user information from request body
    Server->>Server: Construct prompt using financial_info
    Server->>OpenAI API: POST /v1/chat/completions (prompt)
    OpenAI API-->>Server: Response (generated text)
    Server->>Server: Process and format response
    Server-->>Client: Render HTML page with scholarship opportunities
```

1. The client sends a POST request to the `/submit-application` route with the user's information (first name, last name, and financial information) in the request body.
2. The server extracts the user information from the request body.
3. The server constructs a prompt using the `financial_info` provided by the user.
...
6. The server processes and formats the response from the OpenAI API.
7. The server renders an HTML page with the list of scholarship opportunities and sends it back to the client.

Sources: [server.js:39-116]()

## Configuration and Credentials

The server uses the `creds.js` file to store the OpenAI API key:
...
The API key is likely imported and used in the `fetchFromOpenAI` function to authenticate the requests to the OpenAI API.

Sources: [creds.js]()

## Conclusion

The server-side components of this project provide the core functionality of generating personalized scholarship opportunities based on user input. The server integrates with the OpenAI GPT-4 language model to generate tailored recommendations, which are then rendered as an HTML page and sent back to the client. The server is built using Express.js and handles the necessary routing, data processing, and API integration.